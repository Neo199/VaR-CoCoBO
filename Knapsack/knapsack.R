...
knapsack_prob <- function(x_mat) {
  ...
  for (i in seq_len(num_inputs)) {
    res <- knapsack(x_mat[i, ], n_samples, seed = 10)
    cost[i] <- res$fn
  }
  return(cost)
}

knapsack_prob_lang <- function(x_mat, gamma = 10) {
  ...
  for (i in 1:num_inputs) {
    ...
    value_res <- knapsack(x_mat[i, ])
    value <- value_res$fn
    constraint <- sum(value_res$constraint)
    
    # Compute total output
    out[i] <- value - sum(gamma * (constraint - value_res$limit))
  }
  return(out)
}

langmodel <- function(x_vals){
  knapsack_prob_lang(x_vals) 
}
...
# -------------------------
n_vars <- 8                       # number of items / binary variables
...
y_vals <- knapsack_prob_lang(x_vals)
...
plot(c(1:(evalBudget+n_init)), Y, type ='l')
